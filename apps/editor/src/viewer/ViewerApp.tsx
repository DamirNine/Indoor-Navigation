        ...
        <div style={{
          ...
          {filtered.map(n => (
            <div key={n.id}
              ...
              style={{ padding: '8px 12px', cursor: 'pointer', fontSize: 14, borderBottom: '1px solid #f0f0f0', display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontWeight: 500 }}>{n.label}</span>
              <span style={{ color: '#aaa', fontSize: 12 }}>{n.floorName}</span>
            </div>
          ))}
        </div>
      )}
...
export default function ViewerApp() {
  ...
  return (
    <div style={{ display: 'flex', height: '100dvh', fontFamily: 'sans-serif', fontSize: 14, overflow: 'hidden' }}>
      {/* Sidebar — fixed height, only steps section scrolls */}
      <div style={{ width: 300, borderRight: '1px solid #ddd', display: 'flex', flexDirection: 'column', background: '#fafafa', flexShrink: 0, height: '100dvh' }}>

        {/* Header */}
        <div style={{ padding: '14px 16px', background: '#1976D2', color: 'white', flexShrink: 0 }}>
          ...
          <button onClick={() => { localStorage.removeItem(STORAGE_KEY); setBuilding(null); setRoute(null); }}
            style={{ fontSize: 12, color: 'rgba(255,255,255,0.75)', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
            ← загрузить другой
          </button>
        </div>
        ...
        <div style={{ padding: '12px 16px', borderBottom: '1px solid #eee', flexShrink: 0 }}>
          <div style={{ fontSize: 12, color: '#888', marginBottom: 8, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5 }}>Этаж</div>
        ...
        {/* Route search */}
        <div style={{ padding: '12px 16px', borderBottom: '1px solid #eee', flexShrink: 0 }}>
          <div style={{ fontSize: 12, color: '#888', marginBottom: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5 }}>Маршрут</div>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
            <button onClick={handleBuild} disabled={!fromId || !toId}
              style={{ flex: 1, padding: '10px 0', background: fromId && toId ? '#1976D2' : '#e0e0e0', color: fromId && toId ? 'white' : '#aaa', border: 'none', borderRadius: 4, cursor: fromId && toId ? 'pointer' : 'default', fontSize: 14, fontWeight: 600 }}>
              Построить
            </button>
            {route !== null && (
              <button onClick={handleClear}
                style={{ padding: '10px 14px', background: '#f5f5f5', border: '1px solid #ccc', borderRadius: 4, cursor: 'pointer', fontSize: 16 }}>
                ✕
              </button>
            )}
          </div>
          {route !== null && (
            <div style={{ marginTop: 8, fontSize: 13, padding: '6px 10px', borderRadius: 4, background: route.length === 0 ? '#fff3e0' : '#e8f5e9', color: route.length === 0 ? '#e65100' : '#2e7d32' }}>
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', top: 10, right: 10, display: 'flex', gap: 6, zIndex: 10 }}>
          <button onClick={() => setRotation(r => (r + 270) % 360)} title="Повернуть влево"
            style={{ width: 48, height: 48, fontSize: 22, background: 'white', border: '1px solid #ccc', borderRadius: 6, cursor: 'pointer', boxShadow: '0 2px 6px rgba(0,0,0,0.15)' }}>↺</button>
